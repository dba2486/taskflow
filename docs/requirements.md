...
- 사용자 관련 API는 인증된 사용자만 접근할 수 있다.
- 사용자 식별은 JWT Access Token을 기준으로 수행한다.
- 클라이언트는 userId를 직접 전달하지 않는다.
- 사용자 정보 조회/수정/삭제는 `/users/me` 엔드포인트를 기준으로 한다.
- `/users/{id}` 형태의 사용자 조회는 테스트 또는 관리자 기능으로 제한한다.
